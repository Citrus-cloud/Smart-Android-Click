...
object TextClickStore {
    fun load(context: Context): TextClickConfig {
        ...
        return TextClickConfig(
            query = prefs.getString(KEY_QUERY, DEFAULT_QUERY) ?: DEFAULT_QUERY,
    ...
    fun save(context: Context, config: TextClickConfig) {
        ...
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit()
            .putString(KEY_QUERY, safe.query)
...
fun TextClickConfig.normalized(): TextClickConfig = copy(
    query = query.trim().ifBlank { DEFAULT_QUERY }.take(200),
